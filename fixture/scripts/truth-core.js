...
export async function truth(client, email) {
  const { rows: acct } = await client.query(
    `SELECT id, email, full_name, last_seen_ip FROM accounts WHERE email = $1`,
    [email],
  );
  ...
  const { rows: emails } = await client.query(
    `SELECT email FROM account_emails WHERE account_id = $1`,
    [a.id],
  );
  const allEmails = [...new Set(emails.map((e) => e.email).concat(a.email))];

  const one = async (sql, params) =>
    (await client.query(sql, params)).rows[0].count | 0;

  const orders = await one(
    `SELECT count(*)::int AS count FROM orders WHERE account_id=$1`,
    [a.id],
  );
  const items = await one(
    `SELECT count(*)::int AS count FROM order_items i
      JOIN orders o ON o.id = i.order_id WHERE o.account_id=$1`,
    [a.id],
  );
  const refundsSettled = await one(
    `SELECT count(*)::int AS count FROM refunds r
      JOIN orders o ON o.id = r.order_id
     WHERE o.account_id=$1 AND r.status='settled'`,
    [a.id],
  );
  const tickets = await one(
    `SELECT count(*)::int AS count FROM support_tickets WHERE account_id=$1`,
    [a.id],
  );
  const linkedUploads = await one(
    `SELECT count(*)::int AS count FROM uploads WHERE account_id=$1`,
    [a.id],
  );
  const orphanUploads = await one(
    `SELECT count(*)::int AS count FROM uploads
      WHERE account_id IS NULL AND object_key LIKE $1`,
    [`uploads/acct_${a.id}/%`],
  );
  const events = await one(
    `SELECT count(*)::int AS count FROM event_log
      WHERE email = ANY($1) OR ip_address = $2`,
    [allEmails, a.last_seen_ip],
  );
  ...
  const { rows: collisions } = await client.query(
    `SELECT id, email FROM accounts WHERE full_name=$1 AND id<>$2`,
    [a.full_name, a.id],
  );
