...
import {
  createSandboxMinioClient,
  executeSandboxMinioDeletion,
} from './minio-executor.js';
import {
  createPostgresExecutor,
  normalizePostgresRecordType,
...
const finding = {
  system: z
    .enum(SYSTEMS)
    .describe(
      'Which source system holds the record: postgres, minio, or billing. Name the system, not the tool or server you found it through.',
    ),
  record_type: z
    .enum(RECORD_TYPES)
    .describe(
      'What kind of record this is, singular: account, account_email, order, order_item, refund, retained_refund, support_ticket, upload, event (postgres); object (minio); customer (billing).',
    ),
  record_id: z
    .union([z.string(), z.number()])
    .describe('The record primary key, or the object key for a minio object.'),
  locator: z
    .string()
    .max(1000)
    .optional()
    .describe('For minio objects, the full object key.'),
  metadata: z
    .record(z.any())
    .optional()
    .describe(
      'The source row, kept for the audit trail. Pass the row as you received it - its own columns at the top level. A row nested under a "row" key is also accepted, and for an upload the account_id column is what separates a linked object from an orphaned one, so keep it.',
    ),
  disposition: z
    .enum(['erase', 'retain', 'review'])
    .optional()
    .describe(
      'erase by default. retained_refund records are always recorded as retain regardless of what is passed.',
    ),
};
...
async function eraseBillingCustomer({ customerId, caseId, planHash }) {
  ...
  const response = await fetch(
    `${base}/customers/${encodeURIComponent(customerId)}/erase`,
    {
      method: 'POST',
...
export function createRealExecutionInterfaces({
  ...
  return Object.freeze({
    database: ({ plan, case_id, actions, withheld }) =>
      getPostgresExecutor().execute({
        ...(plan || { case_id, actions }),
        withhold: (withheld || []).map((record) => ({
          ...record,
          table: normalizePostgresRecordType(
            record?.record_type ?? record?.table,
          ),
          id: record?.record_id ?? record?.id,
        })),
      }),
    minio: ({ plan, planHash, plan_hash, approval, postgresPhase, withheld }) =>
      executeSandboxMinioDeletion({
        plan,
    ...
    billing: async ({
      ...
      const result = await executeBillingCleanup({
        ...
        postgresTransaction: async () => ({
          manifest: postgresPhase?.result?.manifest || [],
        }),
        billingErase,
      });
      if (!result.ok)
        throw new Error(result.error || 'billing execution failed');
      return result;
    },
  });
}
...
export function createServer({ interfaces = defaultExecutionInterfaces } = {}) {
  ...
  server.registerTool(
    ...
    {
      description:
        'Create an erasure case for one precisely identified subject.',
      inputSchema: {
  ...
  server.registerTool(
    ...
    {
      description:
        'Read a complete case including findings, plans, approvals and certificate.',
      inputSchema: { case_id: caseId },
  ...
  server.registerTool(
    ...
    {
      description:
        'List erasure cases, optionally filtered by lifecycle status.',
      inputSchema: {
        status: z
          .enum([
            'discovered',
  ...
  server.registerTool(
    ...
    {
      description:
        'Record a discovered personal-data record and its intended disposition. Terminal cases cannot be mutated.',
      inputSchema: { case_id: caseId, ...finding },
      annotations: write,
    },
  ...
  server.registerTool(
    ...
    {
      description:
        'Record many discovered records in one call. Prefer this over repeated finding_add: a real subject has hundreds of records, and recording a whole query result in one call is how an investigation stays exhaustive. All findings are inserted in a single transaction, so the batch either lands whole or not at all. Terminal cases cannot be mutated.\n\nTwo forms. Use record_ids with a shared system/record_type/disposition when a whole result set is the same kind of record - hundreds of event-log ids, for example - so the batch is a list of ids rather than a list of repeated objects. Use findings when the rows differ or when each needs its own metadata. Supply exactly one of the two.',
      inputSchema: {
        case_id: caseId,
        findings: z
          .array(z.object(finding))
          .min(1)
          .max(2000)
          .optional()
          .describe(
            'Full findings, one object per record. Use when the records differ or carry metadata.',
          ),
        record_ids: z
          ...
          .optional()
          .describe(
            'Compact form: many record ids that share one system, record_type, and disposition. Requires system and record_type.',
          ),
        system: finding.system
          .optional()
          .describe('System for every id in record_ids.'),
        record_type: finding.record_type
          .optional()
          .describe('Record type for every id in record_ids.'),
        disposition: finding.disposition.describe(
          'Disposition for every id in record_ids. erase by default.',
        ),
      },
      annotations: write,
    },
    async ({
      ...
      const batch =
        findings ??
        (() => {
          if (!system || !record_type)
            throw new Error('record_ids requires system and record_type');
          return record_ids.map((record_id) => ({
            system,
            record_type,
            record_id,
            disposition,
  ...
  server.registerTool(
    ...
    {
      description:
        "Mark discovery complete for a case. Call this only after every discovery finding for the case has been recorded; plan_create refuses to build a plan until this has been called for the case's current findings, so an investigation that aborts partway (for example on a truncated storage query) can never be planned or executed from its partial findings.",
      inputSchema: { case_id: caseId },
      annotations: write,
    },
  ...
  server.registerTool(
    ...
    {
      description:
        "Build and persist a deletion plan from the current findings. Requires case_complete_discovery to have been called for the case's current findings. Concurrent case changes cause plan creation to fail; returns the SHA-256 hash to review.",
      inputSchema: { case_id: caseId },
      annotations: write,
    },
    async ({ case_id }) => {
      ...
      if (!value.discovery_completed_at) {
        throw new Error(
          `case ${case_id} has not completed discovery; refusing to plan from a possibly-partial investigation`,
        );
      }
      ...
      const wrongful = body.actions.filter(
        (action) =>
          ALWAYS_RETAIN_RECORD_TYPES.has(action.record_type) &&
          action.disposition === 'erase',
      );
      if (wrongful.length) {
        throw new Error(
          `refusing to plan deletion of retained record(s): ${wrongful
      ...
      return text({
        ...savePlan(case_id, body, planHash, value.revision),
        body,
        plan_hash: planHash,
      });
    },
  ...
  server.registerTool(
    ...
    {
      description:
        'Record human approval for the latest plan at the current case revision. Terminal cases cannot be mutated and older plan hashes cannot be approved.',
      inputSchema: {
        case_id: caseId,
    ...
    async ({ case_id, plan_hash, approved_by, reason }) =>
      text(recordApproval(case_id, plan_hash, approved_by, reason)),
  );
  ...
  server.registerTool(
    ...
    {
      description:
        'The sole Oubliette-owned destructive entry point. Validates the current canonical, approved plan and executes its erase actions through the configured database, MinIO, and billing interfaces. Retain/review actions are withheld and included in the immutable certificate.',
      inputSchema: {
        case_id: caseId,
        plan_hash: z.string().length(64),
    ...
    async ({ case_id, plan_hash, approved_by }) =>
      text(
        await oublietteExecuteErasure({
          caseId: case_id,
          planHash: plan_hash,
...
if (
  ...
  if (process.env.MCP_TRANSPORT === 'http') {
    startHttpMcp(createServer, {
      name: 'oubliette',
      port: Number(
        process.env.OUBLIETTE_MCP_PORT || process.env.MCP_PORT || 4014,
      ),
    });
  } else {
