...
export const executionInterfaces = Object.freeze({
  database: async () => {
    throw new Error('database execution interface is not configured');
  },
  minio: async () => {
    throw new Error('MinIO execution interface is not configured');
  },
  billing: async () => {
    throw new Error('billing execution interface is not configured');
  },
});
...
function resultFailure(result) {
  return (
    ...
    Number(result.counts?.failed) > 0 ||
    (Array.isArray(result.results) &&
      result.results.some((item) => item?.status !== 'deleted'))
  );
}

function confirmedActions(system, actions, result) {
  if (resultFailure(result))
    throw new Error(`${system} execution did not confirm successful deletion`);
  if (typeof result.deleted === 'number') {
    if (result.deleted !== actions.length)
      throw new Error(
        `${system} execution returned an incomplete deletion result`,
      );
    return actions;
  }
  ...
  if (system === 'database') {
    ...
    for (const action of actions) {
      ...
      if (!table)
        throw new Error(
          `database execution returned an unsupported record type: ${action.record_type}`,
        );
  ...
  if (system === 'minio' && Array.isArray(result.results)) {
    ...
    const expected = new Set(
      actions.map((action) => objectKey(action.locator)),
    );
    if (
      deleted.size !== expected.size ||
      [...expected].some((key) => !key || !deleted.has(key))
    ) {
  ...
  if (system === 'billing' && Array.isArray(result.erased)) {
    ...
    if (
      erased.size !== expected.size ||
      [...expected].some((id) => !erased.has(id))
    ) {
...
function loadExecutionPhase(caseId, planHash, system) {
  const row = db
    .prepare(
      `SELECT result, manifest FROM execution_phases
    WHERE case_id = ? AND plan_hash = ? AND system = ?`,
    )
    .get(caseId, planHash, system);
  if (!row) return null;
...
function readExecutionState(caseId, planHash, approvedBy) {
  ...
  let body;
  try {
    body = JSON.parse(plan.body);
  } catch {
    throw new Error('stored plan is not valid JSON');
  }
  if (
    body.case_id !== caseId ||
    plan.plan_hash !== planHash ||
    hashPlan(body) !== planHash
  ) {
  ...
  if (
    ...
    throw new Error(
      'plan is stale; create and approve a new plan for the current case revision',
    );
  }
  const approval = db
    .prepare(
    ...
    .get(caseId, planHash, subject.revision);
  ...
  if (!Array.isArray(body.actions))
    throw new Error('canonical plan actions are invalid');
  const actions = body.actions.map((action) => {
    ...
    if (!target)
      throw new Error(`no execution interface for system: ${action.system}`);
    const disposition = action.disposition || 'erase';
    if (!['erase', 'retain', 'review'].includes(disposition))
      throw new Error('canonical plan contains an invalid disposition');
...
function recordExecutionCertificate({
  caseId,
  planHash,
  manifest = [],
  withheld = [],
  ...
  const transaction = db.transaction(() => {
    const subject = db.prepare('SELECT * FROM cases WHERE id = ?').get(caseId);
    if (!subject) throw new Error(`case not found: ${caseId}`);
    if (subject.status !== 'executing')
      throw new Error('certificate requires an active execution claim');
    const run = db
      .prepare(
        `SELECT * FROM execution_runs
      WHERE case_id = ? AND plan_hash = ? AND status = 'executing'`,
      )
      .get(caseId, planHash);
    if (!run?.approval_id)
      throw new Error('execution run is not bound to an approval');
    ...
    if (!approval)
      throw new Error(
        'the execution approval is no longer valid for the current plan',
      );
    if (approval.approved_by !== run.approved_by)
      throw new Error(
        'execution approval identity does not match the claimed identity',
      );
    if (
      db.prepare('SELECT 1 FROM certificates WHERE case_id = ?').get(caseId)
    ) {
    ...
    try {
      reviewedPlan =
        typeof plan.body === 'string' ? JSON.parse(plan.body) : plan.body;
    } catch {
    ...
    db.prepare(
      `INSERT INTO certificates (case_id, plan_hash, approved_by, manifest, withheld, executed_at)
      VALUES (?, ?, ?, ?, ?, ?)`,
    ).run(
      caseId,
      planHash,
    ...
    const completedCase = db
      .prepare(
        "UPDATE cases SET status = 'completed', updated_at = ? WHERE id = ? AND status = 'executing'",
      )
      .run(timestamp, caseId);
    if (completedCase.changes !== 1) throw new Error('case completion failed');
    const completedRun = db
      .prepare(
        "UPDATE execution_runs SET status = 'completed', updated_at = ? WHERE id = ? AND status = 'executing'",
      )
      .run(timestamp, run.id);
    if (completedRun.changes !== 1)
      throw new Error('execution run completion failed');
  });
  transaction();
  return hydrate(
...
export async function oublietteExecuteErasure({
  caseId,
  planHash,
  approvedBy = null,
  interfaces = executionInterfaces,
}) {
  if (!caseId || !planHash)
    throw new Error('case_id and plan_hash are required');
  ...
  const eraseManifest = initial.actions.filter(
    (action) => action.disposition === 'erase',
  );
  ...
  db.transaction(() => {
    ...
    const run = db
      .prepare(
        'SELECT * FROM execution_runs WHERE case_id = ? AND plan_hash = ?',
      )
      .get(caseId, planHash);
    if (
      !current ||
      current.revision !== initial.subject.revision ||
    ...
    if (run?.status === 'executing')
      throw new Error('case is already executing');
    if (run?.status === 'completed')
      throw new Error('execution has already completed for this plan');
    const approval = db
      .prepare(
        `SELECT * FROM approvals
      WHERE id = ? AND case_id = ? AND plan_hash = ? AND case_revision = ?`,
    ...
    if (run) {
      db.prepare(
        "UPDATE execution_runs SET status = 'executing', approval_id = ?, approved_by = ?, updated_at = ? WHERE id = ?",
      ).run(approval.id, approval.approved_by, timestamp, run.id);
    } else {
      db.prepare(
        `INSERT INTO execution_runs (case_id, plan_hash, approval_id, approved_by, status, created_at, updated_at)
        VALUES (?, ?, ?, ?, 'executing', ?, ?)`,
      ).run(
        caseId,
        planHash,
    ...
    db.prepare(
      "UPDATE cases SET status = 'executing', updated_at = ? WHERE id = ?",
    ).run(timestamp, caseId);
  })();
  ...
  try {
    for (const name of ['database', 'minio', 'billing']) {
      ...
      const execute =
        typeof adapter === 'function' ? adapter : adapter?.execute;
      if (grouped[name].length && typeof execute !== 'function')
        throw new Error(`${name} execution interface is not configured`);
      ...
      const result = await execute.call(adapter, {
        ...
        postgresTransaction:
          interfaces.postgresTransaction ||
          interfaces.billing?.postgresTransaction,
        billingErase:
          interfaces.billingErase || interfaces.billing?.billingErase,
        postgresPhase: systems.database?.ok
          ? { success: true, result: systems.database.result }
          : null,
      });
    ...
    const certificate = recordExecutionCertificate({
      caseId,
      planHash,
    ...
    return {
      case_id: caseId,
      plan_hash: planHash,
      approved_by: initial.approval.approved_by,
      systems,
      withheld,
      certificate,
    };
    ...
    db.transaction(() => {
      db.prepare(
        "UPDATE cases SET status = 'failed', updated_at = ? WHERE id = ? AND status = 'executing'",
      ).run(failedAt, caseId);
      db.prepare(
        "UPDATE execution_runs SET status = 'failed', updated_at = ? WHERE case_id = ? AND plan_hash = ? AND status = 'executing'",
