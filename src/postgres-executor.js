...
const TABLE_ALIASES = new Map([
  ...
  ['account', 'accounts'],
  ['accounts', 'accounts'],
  ['retained_refund', 'retained_refunds'],
  ['retained_refunds', 'retained_refunds'],
]);
...
function normalizePlan(plan) {
  ...
  const blocked = new Set(
    withheld.map(
      (r) =>
        `${normalizePostgresRecordType(r?.table) || r?.table}:${String(r?.id)}`,
    ),
  );
  ...
  for (const action of plan.actions) {
    ...
    const system =
      action.system == null ? null : String(action.system).toLowerCase();
    if (
      system &&
      !['postgres', 'postgresql', 'database', 'db'].includes(system)
    )
      continue;
    if (action.disposition === 'retain' || action.disposition === 'review')
      continue;
    ...
    if (!table)
      throw new Error(
        `unsupported PostgreSQL record type: ${action.record_type}`,
      );
    ...
    if (!/^\d+$/.test(id) || BigInt(id) <= 0n)
      throw new Error(`invalid ${table} record id: ${id}`);
    const key = `${table}:${id}`;
    if (blocked.has(key))
      throw new Error(`withheld record is not deletable: ${key}`);
...
function sandboxGuard(connectionString) {
  if (process.env.NODE_ENV === 'production')
    throw new Error('PostgreSQL deletion executor is sandbox-only');
...
export function createPostgresExecutor({ pool, connectionString } = {}) {
  ...
  if (!ownedPool) {
    const url =
      connectionString ||
      process.env.POSTGRES_SANDBOX_DATABASE_URL ||
  ...
  return {
    async execute(plan) {
      ...
      try {
        ...
        for (const [table, cast] of TABLES) {
          ...
          const condition =
            table === 'refunds'
              ? "id = ANY($1::int[]) AND status = 'settled'"
              : 'id = ANY($1::' + cast + ')';
    ...
    async close() {
      if (ownedPool && typeof ownedPool.end === 'function' && !pool)
        await ownedPool.end();
    },
  };
}
