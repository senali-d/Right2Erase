...
async function contextFromDatabaseWith(db, caseId, planHash, approvedBy) {
  ...
  const subject = db
    .prepare('SELECT id, revision, status FROM cases WHERE id = ?')
    .get(caseId);
  ...
  const approval = db
    ...
    .get(caseId, planHash, subject.revision, approvedBy);
  if (!approval)
    throw new Error('the current plan has not been approved by approvedBy');
...
function plannedBillingRecords(plan) {
  ...
  return actions.filter(
    (action) =>
      normalizeSystem(action?.system) === 'billing' &&
      action?.record_type === 'customer',
  );
}
...
async function loadBillingProgress(caseId, planHash, customerId) {
  ...
  const row = db
    .prepare(
      `SELECT status, result, error FROM billing_progress
    WHERE case_id = ? AND plan_hash = ? AND customer_id = ?`,
    )
    .get(caseId, planHash, customerId);
  if (!row) return null;
  return { ...row, result: readJson(row.result) };
}

async function saveBillingProgress({
  caseId,
  planHash,
  customerId,
  status,
...
async function loadBillingTransaction(caseId, planHash) {
  ...
  const row = db
    .prepare(
      `SELECT status, result FROM billing_transactions
    WHERE case_id = ? AND plan_hash = ?`,
    )
    .get(caseId, planHash);
  return row ? { ...row, result: readJson(row.result) } : null;
}

async function saveBillingTransaction({
  caseId,
  planHash,
  status,
  result = null,
...
export async function executeBillingCleanup({
  ...
  postgresTransaction,
  billingErase,
  loadContext = contextFromDatabase,
  loadProgress = loadBillingProgress,
  saveProgress = saveBillingProgress,
  loadTransactionProgress = loadBillingTransaction,
  saveTransactionProgress = saveBillingTransaction,
}) {
  const result = {
    ok: false,
    caseId,
  ...
  try {
    if (typeof postgresTransaction !== 'function')
      throw new Error('postgresTransaction is required');
    if (typeof billingErase !== 'function')
      throw new Error('billingErase is required');
    if (typeof planHash !== 'string' || planHash.length !== 64)
      throw new Error('a valid approved plan hash is required');

    const context = await loadContext(caseId, planHash, approvedBy);
    const records = plannedBillingRecords(context.plan);
    if (!records.length)
      throw new Error('approved plan contains no billing customer records');
    ...
    if (!erase.length)
      throw new Error('approved plan contains no billing customers to erase');
    ...
    const committedTransaction = await loadTransactionProgress(
      caseId,
      planHash,
    );
    ...
    for (const record of erase) {
      ...
      try {
        ...
        try {
          ...
          /* preserve the source-system failure */
        }
        throw error;
      }
    }
