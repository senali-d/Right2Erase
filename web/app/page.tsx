  ...
  return (
    <main className="mx-auto max-w-4xl px-6 py-14">
      ...
      <section className="mt-10 rounded border border-line bg-surface p-5">
        <label htmlFor="subject" className="text-[10px] uppercase tracking-[0.14em] text-ink-faint">
          Subject email
        </label>
        <div className="mt-2 flex flex-wrap gap-3">
          <input
            ...
            onKeyDown={(event) => {
              if (event.key === 'Enter' && !busy && email.trim()) void open(false);
            }}
            disabled={busy}
        ...
        {duplicate ? (
          <div className="mt-4 rounded border border-hold/40 bg-hold-dim/40 px-3.5 py-3">
            <p className="text-hold">A case already exists for {duplicate.subject_email}.</p>
            <p className="mt-1 text-[11px] text-ink-faint">
              Cases are permanent audit records with no delete path, so opening a second one for the
              same person cannot be undone.
            </p>
      ...
      <section className="mt-10">
        <h2 className="text-[10px] font-semibold uppercase tracking-[0.14em] text-ink-faint">Cases</h2>
        {cases.length === 0 ? (
