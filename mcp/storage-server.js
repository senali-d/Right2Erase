...
const maxResults = positiveInteger(
  process.env.MCP_STORAGE_MAX_RESULTS || process.env.MCP_MAX_RESULTS,
  1000,
);
  ...
  return new Promise((resolve, reject) => {
    ...
    stream.on('data', (object) => {
      ...
      if (objects.length >= maxResults) {
        ...
        finish(
          new Error(
            `storage listing for ${describe()} exceeds the ${maxResults}-object limit; refusing to return a partial set that would plan an incomplete erasure. Narrow the query, or raise MCP_STORAGE_MAX_RESULTS.`,
          ),
        );
        return;
      }
...
function statObject(key) {
  return client.statObject(bucket, key).then((stat) => ({
    ...
    size: stat.size,
    etag: stat.etag,
    content_type: stat.metaData?.['content-type'] || null,
...
function createServer() {
  ...
  tool(
    ...
    'List all object metadata under a prefix, or fail. Use uploads/acct_<id>/ to find linked and orphaned subject files. The listing is never partial: if it would exceed the server limit the call fails rather than returning a subset, so a result can always be planned from as complete.',
    {
  ...
  tool(
    ...
    'Read metadata for one object. Object content is never returned.',
    {
  ...
  tool(
    ...
    'Search all object keys by a required substring, returning metadata only. Like storage_list_objects, an oversized result fails rather than returning a subset.',
    {
...
if (process.env.MCP_TRANSPORT === 'http') {
  startHttpMcp(createServer, {
    ...
    port: Number(process.env.MCP_STORAGE_PORT || process.env.MCP_PORT || 4013),
  });
} else {
